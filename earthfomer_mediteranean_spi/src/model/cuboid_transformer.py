...
class CuboidTransformerModel(nn.Module):
    ...
    def __init__(self,
        ...
        self.spatial_interpolation = nn.Upsample(size=(H_out, W_out), mode='bilinear', align_corners=False)
    ...
    def get_initial_z(self, final_mem, T_out):
        B, _, H, W, C = final_mem.shape
        if self.z_init_method == 'zeros':
            z_shape = (1, T_out, H, W, C)
            ...
            initial_z = F.interpolate(final_mem.permute(0, 4, 1, 2, 3),
                                    size=(T_out, H, W)).permute(0, 2, 3, 4, 1)
            initial_z = self.z_proj(initial_z)
        elif self.z_init_method == 'last':
            initial_z = torch.broadcast_to(final_mem[:, -1:, :, :, :], (B, T_out, H, W, C))
            initial_z = self.z_proj(initial_z)
    ...
    def forward(self,x, year_float, season_float, verbose=False):
        ...
        eps = 1e-6
        if self.gaussian:
            print("Before exp - min (mean):", out[..., 0:1].min().item(), "max (mean):", out[..., 0:1].max().item())
            print("Before exp - min (std):", out[..., 1:2].min().item(), "max (std):", out[..., 1:2].max().item())

            # For Gaussian distribution, we assume out[..., 0:1] is the mean and out[..., 1:2] is the log of the standard deviation
            mean = out[..., 0:1]
            std = torch.exp(out[..., 1:2])  # exponentiate to ensure the standard deviation is positive

            print("After exp - std min:", std.min().item(), "max:", std.max().item())

            out = torch.cat([mean, std], dim=-1)

            # Print the min and max of the final output
            print("Output - min:", out.min().item(), "max:", out.max().item())
