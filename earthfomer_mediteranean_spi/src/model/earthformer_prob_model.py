...
class CuboidERAModule(pl.LightningModule):
    ...
    def forward(self, batch):
        ...
        output = self.torch_nn_module(input, season_float, year_float)
        mu, sigma = output[..., 0].unsqueeze(-1), output[..., 1].unsqueeze(-1)
    ...
    def training_step(self, batch, batch_idx):
        ...
        # Log NLL loss for each variable
        nll_per_var = self.gaussian_nll_loss(mu, sigma, target).mean(dim=(0, 1, 2, 3))
        for i, nll in enumerate(nll_per_var):
            self.log(f'train_nll_var_{i}', nll, on_step=False, on_epoch=True)
    ...
    def validation_step(self, batch, batch_idx, dataloader_idx=0):
        ...
        if not self.eval_example_only or data_idx in self.val_example_data_idx_list:
            ...
            crps_per_var = self.crps_loss(mu, sigma, target_seq).mean(dim=(0, 1, 2, 3))
            # Calculate CRPS for each variable
            crps_per_var = self.crps_loss(mu, sigma, target_seq).mean(dim=(0, 1, 2, 3))
            for i, crps in enumerate(crps_per_var):
                getattr(self, f'valid_crps_var_{i}')(crps)
            ...
            self.valid_crps(self.crps_loss(mu, sigma, target_seq))
            self.valid_nll(self.gaussian_nll_loss(mu, sigma, target_seq))
...
def main():
    ...
    print("input_shape", input_shape)
    print("output_shape", output_shape)
    prob_output_shape = output_shape + [2]
    print("prob output shape",prob_output_shape)
    ...
    pl_module = CuboidERAModule(
        ...
        output_shape = prob_output_shape)
