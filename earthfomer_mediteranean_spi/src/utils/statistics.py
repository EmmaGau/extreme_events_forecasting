...
class DataScaler:
    def __init__(self, config):
        ...
        # Ajouter l'argument coarse
        self.coarse = config.get("coarse", False)

    def inverse_transform(self, data: xr.Dataset, statistics: xr.Dataset, var: str):
    ...
    def transform(self, data: xr.Dataset, statistics: xr.Dataset, var: str):
        if self.mode == 'standardize':
            return self._standardize(data, statistics, var)
        else:
            raise ValueError(f"Unsupported scaling mode: {self.mode}")
    
    def _standardize(self, data: xr.Dataset, statistics: xr.Dataset, var: str):
        if self.coarse:
            # Utiliser les statistiques globales pour la variable spécifiée
            standardized_data = (data[var] - statistics['mean'][var]) / statistics['std'][var]
        else:
            dayofyear = data.time.dt.dayofyear
            # Standardiser les données pour la variable spécifique
            standardized_data = (
                (data[var] - statistics['mean'][var].sel(dayofyear=dayofyear)) / 
                statistics['std'][var].sel(dayofyear=dayofyear)
            )
        return standardized_data

    def _inverse_standardize(self, data: xr.Dataset, statistics: xr.Dataset, var: str):
        if self.coarse:
            # Utiliser les statistiques globales pour la variable spécifiée
            return (data[var] * statistics['std'][var]) + statistics['mean'][var]
        else:
            dayofyear = data.time.dt.dayofyear
            # Appliquer l'inverse de la standardisation pour la variable spécifique
            return (
                (data[var] * statistics['std'][var].sel(dayofyear=dayofyear)) + 
                statistics['mean'][var].sel(dayofyear=dayofyear)
            )
